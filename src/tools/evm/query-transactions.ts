...
SEE ALSO: portal_get_recent_transactions (simpler, auto-calculates blocks)`,
    {
      ...
      timeframe: z
        .string()
        .optional()
        .describe("Time range (e.g., '24h', '7d'). Alternative to from_block/to_block. Supported: 1h, 6h, 12h, 24h, 3d, 7d, 14d, 30d. Large ranges OK with low limit (<=100)."),
      from_block: z.number().optional().describe("Starting block number (use this OR timeframe). Large ranges OK with low limit (<=100)."),
      to_block: z
        .number()
        .optional()
        .describe(
      ...
      from_addresses: z.array(z.string()).optional().describe("FILTER: Sender addresses (wallets or contracts that initiated the transaction). Optional if limit <=100."),
      to_addresses: z
        .array(z.string())
        .optional()
        .describe("FILTER: Recipient addresses (typically contracts being called, or wallets receiving ETH). Optional if limit <=100."),
      sighash: z
        .array(z.string())
        .optional()
