...
import { getDatasetMetadata, resolveDataset, validateBlockRange } from '../../cache/datasets.js'
import { PORTAL_URL } from '../../constants/index.js'
import { detectChainType, isL2Chain } from '../../helpers/chain.js'
import { portalFetchStream } from '../../helpers/fetch.js'
import { buildEvmLogFields, buildEvmStateDiffFields, buildEvmTransactionFields } from '../../helpers/fields.js'
...
export function registerBatchQueryTool(server: McpServer) {
  server.tool(
    ...
    {
      datasets: z.array(z.string()).min(1).max(10).describe('List of datasets to query (max 10)'),
      query_type: z.enum(['logs', 'transactions', 'balances']).describe('Type of query to execute'),
      from_block: z.number().optional().describe('Starting block (uses last 1000 blocks if not specified)'),
      to_block: z.number().optional().describe('Ending block'),
      addresses: z.array(z.string()).optional().describe('Addresses to filter (contract for logs, from/to for txs)'),
      topic0: z.array(z.string()).optional().describe('Event signatures for log queries'),
      limit_per_dataset: z.number().optional().default(100).describe('Max results per dataset'),
      finalized_only: z.boolean().optional().default(false).describe('Only query finalized blocks'),
    },
    async ({ datasets, query_type, from_block, to_block, addresses, topic0, limit_per_dataset, finalized_only }) => {
      ...
      if (evmDatasets.length === 0) {
        throw new Error('No EVM datasets provided. Batch query currently only supports EVM chains.')
      }
      ...
      const results = await Promise.all(
        evmDatasets.map(async (dataset) => {
          try {
            ...
            const maxBlock = finalized_only && meta.finalized_head ? meta.finalized_head.number : meta.head.number
            ...
            let items: unknown[] = []
            if (query_type === 'logs') {
              items = response.flatMap((block: unknown) => (block as { logs?: unknown[] }).logs || [])
            } else if (query_type === 'transactions') {
              items = response.flatMap((block: unknown) => (block as { transactions?: unknown[] }).transactions || [])
            } else {
              items = response.flatMap((block: unknown) => (block as { stateDiffs?: unknown[] }).stateDiffs || [])
            }
